...
test  ({ id: testId, name: "Таймер в углу - развернуть"}, async ({ runId, page}) => {
    try {
        ...
        await page.waitForTimeout(300);
        await page.waitForSelector("#timer-full-screen", { visible: true });
        await page.click("#timer-full-screen", { delay: 50 });

        await page.waitForTimeout(300);
        await page.waitForSelector("#timer-minimize", { visible: true });
        await page.click("#timer-minimize", { delay: 50 });

        await page.waitForTimeout(300);
        await page.waitForSelector("#minimized-timer-button", { visible: true });
        await page.click("#minimized-timer-button", { delay: 50 });

        const isSuccess = await isElement.exist( page, ".css-jqvmkm" );
